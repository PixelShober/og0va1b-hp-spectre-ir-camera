...
IR_TUNING_CONFIG = "/etc/ir-camera-bridge.ini"
...
class VideoCapture:
	def __init__(self, config):
		...
		# The frame height
		self.fh = None
		self._ir_torch_enabled = False
		self._create_reader()

		self._enable_ir_torch()
	...
	def _i2c_transfer(self, *args):
		cmd = ["/usr/bin/i2ctransfer", "-f", "-y", "2", *args]
		try:
			subprocess.run(
				cmd,
				check=True,
	...
	def _lm3643_write(self, reg, value):
		return self._i2c_transfer("w2@0x63", f"0x{reg:02x}", f"0x{value:02x}")

	def _get_ir_torch_current(self):
		current = self.config.getint("video", "ir_torch_current", fallback=4)
		tuning = configparser.ConfigParser()
		if tuning.read(IR_TUNING_CONFIG) and tuning.has_section("image"):
			current = tuning.getint("image", "ir_torch_current", fallback=current)
		return max(0, min(current, 0x7f))

	def _enable_ir_torch(self):
		"""
		Enable the laptop's TXNW3643/LM3643 IR illuminator while Howdy captures.
		The low torch current keeps faces visible without leaving the emitter on
		when Howdy is idle.
		"""
		if not self.config.getboolean("video", "ir_torch_enable", fallback=True):
			return

		current = self._get_ir_torch_current()

		# Reading flags clears latched faults before enabling a new torch session.
		self._i2c_transfer("w1@0x63", "0x0a", "r1")
		self._i2c_transfer("w1@0x63", "0x0b", "r1")

		if not self._lm3643_write(0x05, current):
			return
		if not self._lm3643_write(0x06, current):
			return
		if not self._lm3643_write(0x01, 0x0b):
			return

		self._ir_torch_enabled = True
		atexit.register(self._disable_ir_torch)
		time.sleep(0.2)

	def _disable_ir_torch(self):
		if self._ir_torch_enabled:
			self._lm3643_write(0x01, 0x00)
			self._ir_torch_enabled = False
